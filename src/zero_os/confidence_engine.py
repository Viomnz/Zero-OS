...
def score_confidence(
    ...
    """Return a heuristic investigation score, never an authority grant.

    Pure Logic invariant: confidence can help rank what to inspect next, but it
    cannot certify truth, scope, safety, or permission to mutate state.
    """
    evidence_score = min(0.3, max(0, int(evidence_count)) * 0.05)
    contradiction_penalty = min(0.4, max(0, int(contradictory_signals)) * 0.12)
    ...
    # A live contradiction is not averaged away. The score may remain useful for
    # investigation priority, but contradiction prevents promotion elsewhere.
    contradiction_active = int(contradictory_signals) > 0
    return {

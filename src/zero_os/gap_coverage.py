...
def zero_ai_gap_status(cwd: str) -> dict:
    ...
    checks = {
        ...
        "backup_ready": int(backup.get("snapshot_count", 0)) > 0 or bool(backup.get("cure_firewall_backup_exists", False)),
        "knowledge_ready": bool(know.get("ok", False)),
        "consciousness_ready": bool(con.get("ok", False)) and int(con.get("ledger_events", 0)) > 0,
        "harmony_ready": bool(harm.get("harmonized", False)),
        "brain_awareness_ready": bool(brain.get("ok", False)) and not bool(brain.get("missing", False)),
    }

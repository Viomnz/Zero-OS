...
def test_v8_identified_runtime_paths_are_constitutionally_integrated():
    root = Path(__file__).resolve().parents[1]
    report = audit_authority_runtime_integration(root)
    missing = [item["requirement_id"] for item in report["requirements"] if not item["satisfied"]]
    ...
    assert report["evidence_kind"] == "AST_CALL_EDGE_PRESENCE"


def test_integration_audit_never_claims_runtime_proof():
    root = Path(__file__).resolve().parents[1]
    report = audit_authority_runtime_integration(root)
    assert "call_presence_is_not_full_reachability_proof" in report["limitations"]
    assert "runtime_trace_attestation_still_required" in report["limitations"]
